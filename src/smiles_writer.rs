...
//! Implementation of a SMILES Writer 
//! 
//! A two-pass traversing algorithm is from the Book "Tutorials in Chemoinformatics", Page 422 - 425, by Alexandre Varnek
//!
...
/// trait Molecule
///     Molecule is a must-have fundamental class of other cheminformatics toolkits
///     use trait for consideration of compatibility
trait MoleculeForSMILES {
...
#[inline(always)]
fn get_neighbours_excluding_parent<T: MoleculeForSMILES>(
    mol: &T,
    atom_current: usize, 
    atom_parent_opt: Option<usize>, 
) -> Vec<usize> {
    mol.get_neighbours_of_atom(atom_current).into_iter()
...
fn get_closures_for_atom<T: MoleculeForSMILES>(
    mol: &T,
    atom_current: usize, 
    atom_parent_opt: Option<usize>, 
    dp: &mut DataPool
...
fn build_smiles_for_atom<T: MoleculeForSMILES>(
    mol: &T,
    atom_current: usize,
    atom_parent_opt: Option<usize>,
    dp: &mut DataPool
    ...
    if let Some(oadts) = dp.closing_closures.get_mut(&atom_current) {
        oadts.sort_by_key(|oadt| oadt.1); // close multiple rings, start from the larger digits
        oadts.reverse();
...
/// SMILES Writer
fn write_smiles<T: MoleculeForSMILES>(
    mol: &T
) -> String {
...
mod tests {
    ...
    impl MoleculeForSMILES for Molecule {
        fn get_neighbours_of_atom(&self, atom: usize) -> Vec<usize> {
    ...
    fn it_works() {
       ...
       for td in test_data.iter() {
           let smiles = td.clone();
            ...
            assert_eq!(write_smiles(&mol), smiles);
       }
    }
}

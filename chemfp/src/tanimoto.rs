//use crate::{bitvec::BitVec, errors::FingerprintError};
//
...
//        return Err(FingerprintError::SizeMismatch(fp1.size(), fp2.size()));
//    }
//
//    let intersection = fp1.and(fp2).count_ones(); // Bits active in both fingerprints
//    let union = fp1.or(fp2).count_ones(); // Bits active in either fingerprint
//
...
//    }
//
//    let intersection = fp1.and(fp2).count_ones();
...
pub fn tanimoto_similarity(fp1: &BitVec, fp2: &BitVec) -> f64 {
    assert_eq!(fp1.size(), fp2.size());

    let intersection = fp1.and(fp2).count_ones();
    let union = fp1.or(fp2).count_ones();

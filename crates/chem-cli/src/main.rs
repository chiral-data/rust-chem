...
use backend::Backend;
use chem_draw::structure::{StructureOptions, StructureTheme};
use chem_draw::svg::structure_to_svg;
use chem_io::reader::Format;
...
/// `chem_io::reader::Format` is not ours to derive `ValueEnum` on, so this is the
    ...
    match &cli.command {
        ...
        Command::Aromatic {
            ...
            for record in &read.outcome.records {
                ...
                chem_io::aromaticity::detect_aromaticity(&mut molecule);
        ...
        Command::Coords {
            ...
            for record in &read.outcome.records {
                ...
                let ok = if *relayout {
                    chem_core::layout::layout(&mut molecule)
                } else {
                    chem_core::layout::ensure_coords(&mut molecule)
                };
                if !ok {
                    failed += 1;
            ...
            match outdir {
                Some(dir) => {
                    ...
                    let renamed = filenames
                        .iter()
                        .zip(&names)
                        .filter(|(f, n)| *f != &chem_draw::svg::suggested_filename(n))
                        .count();
        ...
        Command::Search {
            ...
            let molecule = chem_io::smiles::parse_smiles(query)
...
fn aromatic_atoms(molecule: &chem_core::molecule::Molecule) -> usize {

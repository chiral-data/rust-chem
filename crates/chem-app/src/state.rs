...
use chem_core::layout::ensure_coords;
use chem_core::molecule::Molecule;
use chem_draw::structure::StructureOptions;
use chem_io::aromaticity::detect_aromaticity;
use chem_io::smiles::parse_smiles;
use chem_search::{FingerprintSearch, SearchResult};

#[cfg(target_arch = "wasm32")]
use chem_gpu::{GpuMorganFingerprint, GpuTanimoto};
#[cfg(target_arch = "wasm32")]
...
impl AppState {
    ...
    pub fn apply_loaded_file_bytes(&mut self, name: String, bytes: Vec<u8>) {
        ...
        let outcome = chem_io::reader::read(&content, format);

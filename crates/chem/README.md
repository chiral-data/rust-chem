...
| [`chem::io`](https://docs.rs/chem/latest/chem/io/) | a format registry spanning molecules, trajectories, volumes, meshes and tables — `chem convert -L formats` lists every code this build reads and writes |
...
| `format_registry` | listing what this build can read and write, and checking a pair before converting |
| `trajectory_frame_source` | streaming frames from a trajectory format one at a time, rather than loading them all |
...
chem convert mols.smi -o mols.sdf        # read one format, write another
```
...
- Aromaticity perception is a Hückel heuristic over the SSSR, not a full model.
  ...
  untested.
- Not every format pair round-trips every attribute it claims — `chem convert
  -L formats` shows what a build reads and writes, and `chem convert -L matrix`
  shows what actually survives each pair, attribute by attribute.

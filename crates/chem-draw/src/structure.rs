//! Describing a molecule's 2D structure as drawable primitives.
//!
//! This crate answers *what to draw* and leaves *how to draw it* to whoever
//! asked: [`describe_structure`] returns a list of [`StructureShape`]s, and a
//! backend turns those into egui shapes, an SVG document, or anything else.
//!
//! The types are `emath`'s and `ecolor`'s — [`Rect`], [`Pos2`], [`Vec2`],
//! [`Align2`], `Color32`. Those are separate published crates that egui
//! re-exports, so a GUI consumer gets egui's own types back with no conversion,
//! while a command-line one never links a GUI at all.
//!
//! The one thing a backend must supply is [`MeasureText`]: bonds stop at the
//! edge of their atom labels, so the insets have to be computed with the same
//! font metrics that will draw them.

use chem_core::bond::BondOrder;
use chem_core::geometry::{BoundingBox, Point2};
use chem_core::molecule::Molecule;
use chem_core::rings::find_sssr;
use ecolor::Color32;
...
/// Rendering options for [`describe_structure`].
///
...
mod tests {
    use super::*;
    use chem_core::layout::layout;
    use chem_core::prelude::*;
    use emath::pos2;
    ...
    fn benzene_with_coords() -> Molecule {
        ...
        chem_core::layout::layout(&mut mol);
        mol
    }
